// public/js/rayon-tambah-kegiatan.js

// Impor fungsi-fungsi dari modul auth.js dan api.js
import { updateAuthUI, handleAuthClick, getAuthData } from './auth.js';
import { submitActivity, uploadFile, getAllRayons, registerForActivity } from './api.js'; // Impor API yang diperlukan
...
document.addEventListener('DOMContentLoaded', async function () { // Tambahkan async
    ...
    let currentActivityRegistrations = []; // Menyimpan pendaftar yang ditambahkan secara manual di form ini
    ...
    // Elemen DOM spesifik untuk halaman tambah kegiatan
        ...
        activityImageUploadInput.addEventListener('change', async function() { // Tambahkan async
            if (this.files && this.files[0]) {
                ...
                try {
                    ...
                    this.value = ''; // Bersihkan input file saat gagal
                    showImagePreview(''); // Kembali ke placeholder
                }
            } else {
    ...
    } else if (activityImagePreview.src) { // Check if image preview already has a src (e.g. from previous load)
        activityImagePreview.style.display = 'block';
        activityImageUploadPlaceholder.style.display = 'none';
    }
    ...
    // Logika Pengajuan Formulir Utama
    document.getElementById('addActivityForm')?.addEventListener('submit', async function(e) { // Tambahkan async
        ...
        if (activityImageUrlInput.value) { // Jika ada URL gambar langsung
            imageUrlToSave = activityImageUrlInput.value;
        } else if (activityImageUploadInput.files && activityImageUploadInput.files[0]) { // Jika ada file yang diunggah
            // URL sudah didapatkan dan disimpan di input text oleh handleImageUpload
            imageUrlToSave = activityImageUrlInput.value; 
        ...
        // Get rayon_id and rayon_name from loggedInUser for submission
        let senderRayonId = loggedInUser.rayon_id || null;
        let senderRayonName = loggedInUser.rayon_name || null;

        // Jika rayon_id tidak ada, coba cari dari allRayons berdasarkan nama
    ...
    if (loggedInUser && loggedInUser.role === 'rayon') {
        ...
        if (currentRayon) {
            activityLocationInput.value = currentRayon.address || ''; // Contoh: Pre-fill lokasi dengan alamat rayon
            updateLabelClass(activityLocationInput);
        }
    }
